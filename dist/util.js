...
const log = debug.default('comic-app-server:server');
exports.log = log;
/**
 *
 */
...
function GET(url) {
    return new Promise((resolve, reject) => {
        request.default.get(url, (error, _response, body) => {
            if (error) {
                reject(error);
                return;
            }
...
function getComicLink(body) {
    ...
    return $('div.content_left div.content_grid > ul > li.content_grid_item')
        ...
        .map((liComic) => {
            const $liComic = $(liComic);
            const contentGridItemImg = $liComic.find('div.content_grid_item_img');
            return contentGridItemImg.find('a').attr('href')
        });
}

exports.bodyToComicList = bodyToComicList;
exports.getComicLink = getComicLink;
